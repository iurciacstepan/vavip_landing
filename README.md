...
Запустите встроенный скрипт тестов, чтобы убедиться, что шаблон корректно собран и содержит необходимые файлы:

```bash
./tests/run.sh
```

Скрипт проверяет синтаксис PHP-файла шаблона, валидность манифеста XML, наличие CSS/JS и корректную версию CDN Bootstrap.
